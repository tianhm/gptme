...
``base_url`` and the key from ``api_key_env``. Plugin providers are initialised
on first use and routed through the OpenAI client path.
...
7. **Update docs** — add the provider to ``docs/providers-supported.rst`` and
   the overview table in ``docs/providers.rst``.
...
- :doc:`providers-supported` — built-in providers and their API keys
