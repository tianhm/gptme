    ...
    *,
    ...
    if can_read_files:
        lines += [
            "## File access",
            "",
            "You have the `read` tool with access to an exported snapshot of this",
            "repository at the verified commit. Use repository-relative paths:",
            "`read` resolves them from GPTME_READ_ROOT; reading `.` lists that root.",
            "The process working directory is a separate isolated runtime directory.",
            "Do not construct absolute repository paths from the working directory.",
            "Before quoting source in a finding, open the file and read the real",
            "text — do not reconstruct it from the diff",
