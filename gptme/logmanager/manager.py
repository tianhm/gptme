...
# Number of assistant turns to keep reasoning-heavy messages before pruning.
...
def _auto_tag_ephemeral(msg: Message) -> Message:
    """Return a copy of *msg* with ephemeral_ttl set if the content contains thinking blocks.

    Thinking blocks (<think> / <thinking>) are high-churn content generated by
    reasoning-capable models.  They're valuable for the next couple of assistant
    turns but rarely useful after that, making them the primary target for
    ephemeral pruning.

    Does not overwrite an existing ephemeral_ttl set by the caller.
    """
    if msg.ephemeral_ttl is not None:
        return msg
    if _THINKING_PATTERN.search(msg.content):
        return msg.replace(ephemeral_ttl=_THINKING_TTL)
    return msg
...
class LogManager:
    ...
    def append(self, msg: Message) -> None:
        ...
        # Auto-tag assistant messages containing thinking blocks as ephemeral.
        # Thinking blocks are high-churn and rarely valuable after a few turns,
        # so they're the primary target for ephemeral pruning.
        if msg.role == "assistant" and msg.ephemeral_ttl is None:
            msg = _auto_tag_ephemeral(msg)
